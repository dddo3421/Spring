...
public class PerfomanceAspect {
	//where:aop_01.com.spring_aop.annotationex 패키지 하위의 모든 클래스에 advice 적용
	@Pointcut("within(aop_01.com.spring_aop.annotationex.*)")
	...
	public Object trace(ProceedingJoinPoint joinPoint) throws Throwable{
		//Jointpoint : 특정포인트(핵심 기능 메소드 호출 시 삽입)
		...
		long startTime = System.nanoTime(); //현재 시간과는 상관없이 시작시간과 종료시간의 차이로 수행시간 계산
		...
		curTime = sdf.format(curLong);
		System.out.println("-----------------------------------------------------------------");
		System.out.println("[Log] After : " + methodName + "() 실행 종료");
		System.out.println("[log] 종료시간 " + curTime);
