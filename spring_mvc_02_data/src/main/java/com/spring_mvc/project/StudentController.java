...
public class StudentController {
	...
	public String insertStudent3(Student student
			//,Model model
			) {//폼태그 통해 전달된 데이터가 객체로 자동주입 -> model 객체에도 자동저장
		System.out.println(student.getNo());
		System.out.println(student.getName());
		System.out.println(student.getYear());
		//model.addAttribute("Student", student);
		//command 객체 통해서 파라미터 값 바로 추출해서 객체에 저장 가능한 이유?
		//파라미터명과 객체이 필드명이 동일하기 때문에 모듈이 파라미터명과 필드명으로 매칭을 시킴
		//두 name이 다르면 매칭되지 않아 값 활용 불가능
		return "student/studentResult2";
	}
	
	//4. view로 전달되는 command 객체 명 변경 어노테이션 @ModelAttribute
	...
	// uri 구성시 {변수명} -> 파라미터명처럼 변수명이 사용되어져서 stdNo위치의 문자열을 전송 value로 사용 가능
	...
	public String studentDetailView(
						...
						@PathVariable String stdYear) { //url로 전달된 값을 사용 @PathVariable 어노테이션 사용
		System.out.println(stdNo);
